...
export class EnemyPrsan {
  ...
  draw(ctx) {
    ...
    ctx.strokeStyle = '#edf0df'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(29, -31); ctx.quadraticCurveTo(35, -28, 36, -22); ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.5;
    oval(66, 3, 15, 23, '#d6a057');
    oval(72, 3, 9, 19, '#586761');
    shape('#e4decc', () => {
    ...
    ctx.strokeStyle = '#a6b4a7'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(-33, 23); ctx.quadraticCurveTo(-5, 26, 21, 17); ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.5;
    ...
    for (let trail = 3; trail >= 0; trail -= 1) {
      ...
      ctx.fillStyle = '#e4decc'; ctx.strokeStyle = '#52605e'; ctx.lineWidth = 1.7;
      ctx.beginPath(); ctx.ellipse(0, 0, 5, 33, 0, 0, TAU); ctx.fill(); ctx.stroke();
      ctx.restore();
    }
