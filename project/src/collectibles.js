...
export class CollectibleManager {
  constructor(onCollect) { this.onCollect = onCollect; this.items = []; this.spawnTimer = 0; this.nextSpawn = .8; this.coffeeTimer = 0; this.nextCoffee = 3 + Math.random() * 2; }
  ...
  collect(bounds) { const out = []; for (const item of this.items) { const r = item.size * .38; const x = Math.max(bounds.left, Math.min(item.x, bounds.right)); const y = Math.max(bounds.top, Math.min(item.y, bounds.bottom)); if ((item.x-x)**2 + (item.y-y)**2 < r*r) { item.collected = true; out.push(item); this.onCollect?.(item); } } return out; }
...
function drawBeer(c,x,y,s){c.save();c.translate(x,y);c.scale(s,s);c.shadowColor="rgba(245,184,46,.55)";c.shadowBlur=18;c.fillStyle="#f5b82e";c.strokeStyle="#fff0bf";c.lineWidth=3;c.beginPath();c.roundRect(-13,-15,23,31,5);c.fill();c.stroke();c.shadowBlur=0;c.beginPath();c.roundRect(9,-9,11,18,5);c.stroke();c.fillStyle="#fff4cb";c.beginPath();c.roundRect(-14,-17,25,8,4);c.fill();c.restore();}
function drawCoffee(c,x,y,s){c.save();c.translate(x,y);c.scale(s,s);c.strokeStyle="#d9b38a";c.lineWidth=3;c.beginPath();c.moveTo(-7,-18);c.quadraticCurveTo(-12,-25,-6,-29);c.moveTo(3,-18);c.quadraticCurveTo(8,-25,3,-29);c.stroke();c.fillStyle="#80502f";c.strokeStyle="#e8c39a";c.lineWidth=3;c.beginPath();c.roundRect(-14,-10,25,20,4);c.fill();c.stroke();c.beginPath();c.arc(14,0,7,-Math.PI/2,Math.PI/2);c.stroke();c.restore();}
